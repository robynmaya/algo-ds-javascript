/* 
Return one pair of integers from the array that sum up to the target number
...
const twoNumberSum = (array, targetSum) => {
    ...
    while (left < right) {
        ...
        if (currentSum === targetSum) {
            // loop is done as soon as a pair is found
...
/* Return EVERY pair of integers from the array that sum up to the target number
Solution O(nlog(n)) time | O(1) space
...
const allPairSum = (array, targetSum) => {
    array.sort((a,b) => a - b);
    let left = 0;
    let right = array.length - 1;
    let result = [];
